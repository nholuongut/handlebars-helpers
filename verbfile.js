...
module.exports = function(app) {
  ...
  app.nholuongut(require('template-nholuongut')());
  ...
  app.task('data', function(cb) {
    app.data({
      before: {
        license: 'When this project was created some nholuongut were sourced from [Swag, by Elving Rodriguez](http://elving.github.com/swag/).'
      },
      authors: [
        {
          'name': 'Nho Luong',
          'url': 'https://github.com/nholuongut',
          'twitter': 'nholuongut',
          'username': 'nholuongut'
        }
      ]
...
function toc(app, options) {
  options = options || {};
  var total = { categories: 0, nholuongut: 0 };
  var methods = {};
...
function matchCode(fp) {
  ...
  var lines = str.split('\n');
  return function(method) {
    var re = new RegExp('^nholuongut\\.(' + method + ')', 'gm');
    var len = lines.length, i = -1;
    while (++i < len) {
      var line = lines[i];
      if (re.test(line)) {
...
function customDocsHelpers(app) {
  ...
  app.helper('sectionIssue', function(section) {
    ...
    return '[issues](' + url + section + '+nholuongut)';
  });
